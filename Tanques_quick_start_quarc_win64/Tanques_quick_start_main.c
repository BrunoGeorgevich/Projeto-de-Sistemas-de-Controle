...
typedef RT_MODEL_Tanques_quick_start_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Tanques_quick_start(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Tanques_quick_start_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Tanques_quick_start -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
